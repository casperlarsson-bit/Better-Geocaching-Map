...
if (url.includes('www.geocaching.com/map')) {
...
} else if (url === 'https://www.geocaching.com/p/default.aspx?tab=geocaches#profilepanel') {
	// Get old cache find list instead WIP, only works for my profile
	// FINDS
	...
	// 	const match = wptArray.find((wpt) => wpt.name === titleText)
	// 	if (match) {
	// 		link.href = `https://www.geocaching.com/seek/nearest.aspx?tx=${match.hash}&u=${username}`
	// 	}
	// })
}
